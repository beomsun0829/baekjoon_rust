...
fn main() {
    ...
    let mx: i32 = 100000;
    let mut points: Vec<i32> = vec![i32::MAX / 3; mx as usize + 10];

    let mut deque: VecDeque<(i32, i32)> = VecDeque::new();
    deque.push_back((n, 0));

    let mut ans_dist = i32::MAX / 3;
    let mut ans_ways = 0;
    ...
    while let Some((now, dist)) = deque.pop_front() {
        ...
        if now * 2 >= 0 && now * 2 <= mx && points[now as usize * 2] >= dist + 1 {
            points[now as usize * 2] = dist + 1;
            deque.push_back((now * 2, dist + 1));
        }
        if now - 1 >= 0 && now - 1 <= mx && points[now as usize - 1] >= dist + 1 {
